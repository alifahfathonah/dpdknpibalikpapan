...
class Galeri_model extends CI_Model {
	...
	{
        if ($select == "all") {
            return $this->db->order_by('idglr','DESC')->get($this->_table)->result();
        }
        elseif ($select == "where") {
            return $this->db->where('idglr',$param)->get($this->_table)->row();
        }
        elseif ($select == "count") {
            return $this->db->get($this->_table)->num_rows();
        }
        elseif ($select == "limit") {
            return $this->db->order_by('idglr','DESC')->limit($param)->get($this->_table)->result();
        }
    }
    ...
    public function action($param,$data=array(),$id=null)
	{
		...
		elseif ($param == 'update') {
            return $this->db->where('idglr',$id)->update($this->_table, $data);
        }
		elseif ($param == 'delete') {
            return $this->db->delete($this->_table, array('idglr' => $id));
        }
    }
